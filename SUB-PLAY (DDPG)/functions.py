...
def store_tran_rate(args, mask_rate, store_rate_, replace):
    ...
    else:  # args.mask_type == "distance" or "region"
        ...
        for sub in range(args.num_victim + 1):
            if tran_type == sub:
                ...
                if store_rate[sub] == 0 or store_rate[sub] == 1:
                    ...
                    if tran_rate[tran_type, sub] > 1:
                        print(tran_rate[tran_type, sub])
...
def partial_mask(args, env_agents, obs_n):
    obs_adv = []  # obs_adv can be used for the mask vector generation and subsequent judgment in sub games
    mask = []
        ...
        for i in range(args.num_agents):
            if i >= args.num_victim:
                ...
                for j in range(args.num_victim):
                    pos = 8 + j * 2
                    if sum(abs(obs_n[env_agents[i]][pos:pos + 2])) >= args.distance_range:
    ...
    for i in range(args.num_attacker):
        ...
        # the mask vector is determined by the environment
...
def reward_unified(reward_, env_agents, args):
    ...
    rew_vic = rew_n[:args.num_victim]
    rew_att = rew_n[args.num_victim:]
    rew_min = min(rew_att)
    for i in range(args.num_attacker):
        num_catch += abs(round(rew_att[i] / 100))
        rew_att[i] = rew_min
    rew_ = np.append(rew_vic, rew_att)
