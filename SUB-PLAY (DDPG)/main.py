...
class Runner_DDPG:
    def __init__(self, args):
        ...
        self.reward_buffer = pd.DataFrame(
            np.zeros((self.args.max_episode, self.args.num_agents)), columns=self.env_agents)
        self.reward_vic = []
        self.reward_att = []
        self.num_catch = []
        self.catch_rate = []
    ...
    def stat(self, update=False):
        if not update:
            # the observable probability of the victim before attack
            ...
            # update the experience dissemination table
        ...
        for episode_step in range(self.args.episode_limit):
            ...
            if not evaluate:
                ...
                for sub in range(self.args.num_victim + 1):
                    ...
                    index = 0
                    for agent in agents:
                        if index >= self.args.num_victim:
                            tran_rate = self.tran_rate[int(sum(obs_adv[index - self.args.num_victim])), sub]
                            tran_judge = np.random.choice([0, 1], size=1, p=[1 - tran_rate, tran_rate])

                            if tran_judge == 1:
                                keys = list(self.obs_n.keys())
                                agent.buffer.store_transition(self.obs_n[keys[index]], act_n[keys[index]],
                                                              rew_n[keys[index]], obs_n_[keys[index]],
                                                              done_n[keys[index]])

                                if (agent.buffer.pointer - 1) >= args.memory_capacity and \
                                   (agent.buffer.pointer - 1) % args.episode_limit == 0:
                                    agent.train()
                                    self.num_train += 1
...
if __name__ == '__main__':
    ...
    parser.add_argument("--mask_type", type=str, default="distance", help="random / distance / region")
    parser.add_argument("--mask_rate", type=float, default=0.0, help="probability that the target cannot be observed")
    parser.add_argument("--distance_range", type=float, default=0.0, help="visible distance")
    ...
    for seed in range(3):
        ...
        args.save_dir = "./data/seed{}/result_{}/model".format(args.seed, mask_rate)
        args.load_dir = "./data/seed{}/result_{}/model".format(args.seed, mask_rate)
        args.reward_dir = "./data/seed{}/result_{}/reward".format(args.seed, mask_rate)
